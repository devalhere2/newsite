<script>
    ...
    async function handleFileChange(event) {
        selectedFiles = [...event.target.files].filter((file) => {
            ...
            return (ext === "jpg" || ext === "jpeg") && file.size <= 1000000;
        });
        if (selectedFiles.length > 0) {
            ...
            alert("Please select a .jpg file less than 1MB");
        }
            ...
            try {
                ...
                for (let fileIndex in resolvedFiles) {
                    ...
                    const response = await fetch(
                        `http://127.0.0.1:8000/jpg_to_png`,
                        {
                            method: "POST",
                            body: formData,
                        }
                    ...
                    if (response.ok) {
                        ...
                        a.href = data.image;
                        a.download = `${file.name.split(".")[0]}-convert.png`;
